...
def calculate_mean(dataset):
    ...
    for img_name in dataset:
        # Read the image 
        img_ = cv2.imread(img_name, cv2.IMREAD_GRAYSCALE)
        if len(img_.shape) == 3:
...
glioma_path = root_path + "Glioma/"
healthy_path = root_path + "Healthy/"
meningioma_path = root_path + "Meningioma/"
pituitary_path = root_path + "Pituitary/"
# In[] Create list from root_path
def create_list(path):
    dataset_path = []
    for dirPath, dirNames, fileNames in os.walk(path):
        for file in fileNames:
            if file.endswith('.jpg' ) or file.endswith('.tif') or file.endswith('.png'):
                full_path = dirPath + "/" + file
                dataset_path.append(full_path)
    return dataset_path

# In[] Create list for training, val and test datasets
glioma_list = create_list(glioma_path)
healthy_list = create_list(healthy_path)
meningioma_list = create_list(meningioma_path)
pituitary_list = create_list(pituitary_path)

datasets = [glioma_list, healthy_list, meningioma_list, pituitary_list]
